            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        var values: [String: String] = posts[postKey] as! [String: String]
                        var newPost = Post(id: postKey, username: values["username"]!, postImagePath: values["imagePath"]!, thread: values["thread"]!, dateString: values["date"]!, read: ids.contains(postKey))
